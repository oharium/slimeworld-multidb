...
public class SWMPlugin extends JavaPlugin implements SlimePlugin {
    ...
    private final ExecutorService worldGeneratorService = Executors.newFixedThreadPool(
            Runtime.getRuntime().availableProcessors(),
            new com.google.common.util.concurrent.ThreadFactoryBuilder()
                    .setNameFormat("SWM-WorldGen-%d")
                    .setDaemon(true)
                    .build()
    );
    ...
    public void onDisable() {
        // Shutdown world generator service
        worldGeneratorService.shutdown();
        try {
            if (!worldGeneratorService.awaitTermination(10, java.util.concurrent.TimeUnit.SECONDS)) {
                worldGeneratorService.shutdownNow();
            }
        ...
        // Close all SQL connection pools
        for (String loaderName : new String[]{"mysql", "mariadb", "postgresql"}) {
            SlimeLoader loader = LoaderUtils.getLoader(loaderName);
            if (loader instanceof com.grinderwolf.swm.plugin.loaders.sql.SqlLoader) {
                ((com.grinderwolf.swm.plugin.loaders.sql.SqlLoader) loader).close();
            }
        }
        
        Logging.info("SlimeWorldManager disabled successfully");
    }
