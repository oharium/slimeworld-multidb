...
public class LoaderUtils {
    ...
    public static void registerLoaders() {
        ...
        // MySQL loader
        DatasourcesConfig.SqlConfig mysqlConfig = config.getMysqlConfig();
        if (mysqlConfig.isEnabled()) {
            try {
                registerLoader("mysql", new com.grinderwolf.swm.plugin.loaders.sql.SqlLoader(
                        mysqlConfig, 
                        com.grinderwolf.swm.plugin.loaders.sql.SqlLoader.DatabaseType.MYSQL,
                        "mysql"
                ));
        ...
        // MariaDB loader
        DatasourcesConfig.SqlConfig mariadbConfig = config.getMariadbConfig();
        if (mariadbConfig.isEnabled()) {
            try {
                registerLoader("mariadb", new com.grinderwolf.swm.plugin.loaders.sql.SqlLoader(
                        mariadbConfig, 
                        com.grinderwolf.swm.plugin.loaders.sql.SqlLoader.DatabaseType.MARIADB,
                        "mariadb"
                ));
            } catch (SQLException ex) {
                Logging.error("Failed to establish connection to the MariaDB server:");
                ex.printStackTrace();
            }
        }

        // PostgreSQL loader
        DatasourcesConfig.SqlConfig postgresConfig = config.getPostgresConfig();
        if (postgresConfig.isEnabled()) {
            try {
                registerLoader("postgresql", new com.grinderwolf.swm.plugin.loaders.sql.SqlLoader(
                        postgresConfig, 
                        com.grinderwolf.swm.plugin.loaders.sql.SqlLoader.DatabaseType.POSTGRESQL,
                        "postgresql"
                ));
            } catch (SQLException ex) {
